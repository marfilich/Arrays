...
int task14(int size) {
    ...
    put_elements(calc_elements_square(ptr_array, size), size);
...
int task16(int size) {
    ...
    full_elements_random(ptr_array_16, size);
    put_elements(ptr_array_16, size);

    // Задание 2. Преобразование массива
    int new_size = -1;
    double* new_array = delete_more_than_a(ptr_array_16, size, a, new_size);
    if (new_array == NULL)
    {
        puts("error");
        return -1;
    ...
    put_elements(negative_array, size);
    ...
    ptr_array_a = full_elements_random(ptr_array_a, size_a);
    ...
    ptr_array_b = full_elements_random(ptr_array_b, size_b);
    ...
    ptr_array_c = full_elements_random(ptr_array_c, size_c);
    ...
    puts("Массив C");
    put_elements(ptr_array_c, size_c);
    puts("Массив D");
